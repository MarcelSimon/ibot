...
class Mlp(nn.Module, ListForwardMixin):
    def __init__(
        ...
        self.fc1 = nn.Linear(in_features, hidden_features, bias=bias).to(device=device)
        self.act = act_layer()
        self.fc2 = nn.Linear(hidden_features, out_features, bias=bias).to(device=device)
...
class SwiGLUFFN(nn.Module, ListForwardMixin):
    def __init__(
        ...
        self.w1 = nn.Linear(in_features, swiglu_hidden_features, bias=bias).to(device=device)
        self.w2 = nn.Linear(in_features, swiglu_hidden_features, bias=bias).to(device=device)
        self.w3 = nn.Linear(swiglu_hidden_features, out_features, bias=bias).to(device=device)
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.proj = nn.Linear(dim, dim, bias=proj_bias).to(device=device)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        layerscale_init: Union[float, None] = 1e-5,
...
class DinoV3VisionTransformer(VisionTransformer):
    def __init__(self,
                 img_size,
                 patch_size,
                 embed_dim,
    ...
    def forward(self, x):
        features = list(super().get_intermediate_layers(x, n=self.out_indices, reshape=True, norm=True))
